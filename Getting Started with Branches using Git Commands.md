...
Step 7 - (Before commit the changes, need to tell git who we are.):

# (git config)
Used to configure various settings and options for Git. (sets your global useremail and username for Git) 
# (--global)
Flag indicates that this configuration is applied globally for all repositories on your system.
# (user.)
...
# (git checkout)
command followed by the name of the branch you want to switch to, Git will update your working directory and switch the active branch to the specified branch "my1stbranch".
...
Step (9 and 11) - (AS a short cut to creating branch(my1stbranch) using git branch and then making it active using 'git checkout'):
